...
class DecisionNode(Node):
    def __init__(self):
        ...
        self.fill_level_subscriber = self.create_subscription(
            Float32MultiArray, '/bin_fill_levels', self.fill_level_callback, 10)
        
        self.nav_status_subscriber = self.create_subscription(
            String, '/navigation_executor_status', self.navigation_status_callback, 10)
    ...
    def navigation_status_callback(self, msg: String):
        nav_status = msg.data
        self.get_logger().info(f"Received Nav Status: '{nav_status}' (Phase: '{self.current_task_phase}')")
    ...
    def make_decision_cycle(self):
        ...
        if self.current_sim_pose is None:
            self.get_logger().warn("Decision cycle: Cannot make distance-based decision, sim pose unknown.")
            return
        ...
        for bin_index in candidate_bins:
            ...
            self.get_logger().info(f"Checking candidate bin {bin_index}: Distance = {dist:.2f}m")
        ...
        if closest_bin_index is not None:
            ...
            self.get_logger().info(f"Decision: Dispatched robot to CLOSEST full bin: "
                                f"{self.bins_data[self.dispatched_bin_index]['name']} (Index: {self.dispatched_bin_index})")
